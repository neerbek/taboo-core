...
def initialize_model(num_trees=1000):
    ...
    server_enron.serverState = server_enron_helper.ServerState(
        max_embedding_count=num_wordvectors, glove_path="tests/resources/glove/")
    ...
    server_enron.keywordState.initialize(
        server_enron.serverState.server_rnn_state)
...
class ServiceTest(unittest.TestCase):
    ...
    def test_run_rnn3(self):
        initialize_model()
        server_enron.request = Request()
        server_enron.request.args.args[
            'text'] = "Sentence fragments are representative of their quoted statement"
        res = server_enron.run_rnn()
        if not isinstance(res, str):
            self.assertTrue(False, "expected string response")
        print(res)
        ...
        self.assertTrue(len(d) == 1)
        print(d['predictions'])
        self.assertEqual(1, d['predictions'][0]['prediction'],
                         "expected first result to be sensitive")
    ...
    def test_load_model2(self):
        ...
        req.args.args['data_dir'] = os.path.join(
            os.getcwd(), 'tests/resources/enron_shortdata_test')  # '/home/neerbek/Dropbox/DLP/trec/legal10'
    ...
    def test_load_model3(self):
        initialize_model()
        req = Request()
        req.args.args['data_file'] = os.path.join(os.getcwd(),
                                                  'trees/train.txt')
        ...
        self.assertEqual(600,
                         len(server_enron.serverState.train_trees),
                         "number of new training trees does not match")

    def test_train_rnn1(self):
        initialize_model()
        server_enron.request = Request()
        trainer = server_rnn.Trainer()
        server_enron.serverState.server_rnn_state.load_trees(
            trainer, max_tree_count=100)

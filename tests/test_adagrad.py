...
class RNNWrapper:
    ...
    def create_rnn(self):
        self.rnn = nn_model.RNN(self.rng, self.x, self.z, n_in = self.n_in, n_hidden= self.n_hidden, n_out = self.n_out)
        ...
        self.params = [self.rnn.reluLayer.W, self.rnn.reluLayer.b, self.rnn.regressionLayer.W, self.rnn.regressionLayer.b]
    ...
    def create_data(self):
        ...
        self.x_val = self.rng.randint(n_input_max_range, size=(self.n_examples,self.n_in))
        self.x_val = self.x_val.astype(dtype=theano.config.floatX)
        truth_val = self.rng.randint(self.n_out, size=self.n_examples)
        self.y_val = numpy.zeros(shape=(self.n_examples, self.n_out), dtype=theano.config.floatX)
        for i in range(len(truth_val)):
            self.y_val[i][truth_val[i]] = 1
        self.y_val = self.y_val.astype(dtype=theano.config.floatX)

        self.z_val = self.rng.binomial(n=1, size=(self.x_val.shape[0], self.n_hidden), p=self.retain_probability)

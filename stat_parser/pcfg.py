...
class PCFG:
    ...
    def __init__(self):
        self.q1: Dict[str, float]; self.q1 = defaultdict(float)
        self.q2: Dict[str, float]; self.q2 = defaultdict(float)
        self.well_known_words: Set[str]; self.well_known_words = set()

    def norm_word(self, word):
        return word if word in self.well_known_words else word_class(word)

    def __build_caches(self):
        self.N = set()  # type: Set[str]
        self.binary_rules: Dict[str, List[Tuple[str, str]]]
    ...
    def learn_from_treebanks(self, treebanks):
        self.sym_count = Counter()     # type: Counter
        self.unary_count = Counter()   # type: Counter
        self.binary_count = Counter()  # type: Counter
        self.words_count = Counter()   # type: Counter
        ...
        for (x, y1, y2), count in self.binary_count.items():
            self.q2[x, y1, y2] = self.binary_count[x,
                                                   y1, y2] / self.sym_count[x]

...
def write_batch_to_file(s, index, prefix, maximum):
    util.write_file(sentences=s, filename="{}_{}.txt".format(
        prefix, index), maximum=maximum, do_log=False)
...
def write_file(s, batch, prefix, maximum=-1):
    ...
    max_batch = l // batch
    for i in range(max_batch + 1):
        write_batch_to_file(s[i * batch:(i + 1) * batch],
                            i, prefix, maximum=maximum)
        if i % 500 == 0:
...
labelfile = os.path.join(
    enron_dir, "legal10-results/labels/qrels.t10legallearn")
...
documents: List[EnronDocument.EnronText]
documents = []
...
for i, d in enumerate(doc2):
    ...
    w = []
    for line in s:  # remove empty sentences
        line.strip()
        if len(line) != 0:
            w.append(line)
    if len(w) != 0:
    ...
    if i % 100 == 0:
        print(i, "time elapsed is: {}. Splitting text={}".format(
            time.time() - start, split_time))
        # load_time = 0
        # split_time=0
done = time.time()
print("Number of sentences: YES={}. NO={} ".format(
    len(sentences_yes), len(sentences_no)))
...
sentences_yes[:] = [s.replace("\n", " ").replace(
    "\"", "").replace(":", ";") for s in sentences_yes if keep(s)]
sentences_no[:] = [s.replace("\n", " ").replace(
    "\"", "").replace(":", ";") for s in sentences_no if keep(s)]
print(len(sentences_yes), len(sentences_no))
    ...
    def run(self, work):
        ...
        for i, l in enumerate(it):
            ...
            if len(s) == 0:
                count += 1
                if count % 3 == 0:
                    print("{} completed another tree (not adding yet ({}))".format(
                        self.name, count))
                trees.append(t)
                t = ""
                continue
            t += " " + s
        if error_msg == None:
            print("{} completed file {}. Adding {} trees".format(
                self.name, filename, len(trees)))
            for t in trees:
...
class ReadTrees(ParaphraseThreadState.ThreadContainer):
    ...
    def run(self, work):
        ...
        if i < len(line) - 1:  # Not all of line parsed
            raise Exception(
                self.name + " parsing line failed. There was more than one tree in the line. {}".format(i))
...
tree = ReadTrees(tname, readerTstate, tstate)
readerTstate.add_thread(tname, tree)
readerTstate.start_threads()


# to here

trees: List[load_trees.Node]; trees = []
all_trees = []
...
def get_tree():
    if not readerTstate.completed_work.empty():
        treeCount = readerTstate.completed_work.qsize()
        print("number of trees", treeCount)
        for i in range(l):
            t = readerTstate.completed_work.get(NO_BLOCK)
            trees.append(t)
    else:
        print("no new trees")
    print("Number of trees: {}, number of errors {}, number of getter errors {}".format(
        len(trees), readerTstate.failed_work.qsize(), tstate.failed_work.qsize()))

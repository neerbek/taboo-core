...
def run_rnn():
    ...
    if text is None or len(text) == 0:
        ...
        res = server_enron_helper.index_sentences_to_predictions(
            indexed_sentences)
...
def run_keyword():
    ...
    if text is None or len(text) == 0:
        ...
        print("getting predictions")
        indexed_sentences = inference_enron.get_predictions(
            text, keywordState.indicators)
        res = server_enron_helper.index_sentences_to_predictions(
            indexed_sentences)
        return response_success(res)
...
def load_model_data():
    ...
    if is_none([
            data_dir, labelfile, data_label, total_ratio_str, train_ratio_str,
            dev_ratio_str
    ]):
        return response_error(
            "Load data needs parameters labelfile, data_dir, data_label, total_ratio, train_ratio, dev_ratio"
        )
    ...
    if not is_positive([total_ratio, train_ratio, dev_ratio]):
        return response_error(
            "Load data needs parameters total_ratio, train_ratio, dev_ratio to be postive numbers (below 1)"
        )

#    doc2 = load_labeled_documents("enron_labels.txt","/home/neerbek/Dropbox/DLP/trec/legal10", "201")
    doc2 = server_enron_helper.load_labeled_documents(labelfile, data_dir,
                                                      data_label)
    ...
    parserStatistics = rnn_enron.ParserStatistics()
    train_trees = server_rnn_helper.get_trees(
        doc2[:train_index], serverState.server_rnn_state.LT, parserStatistics)
    dev_trees = server_rnn_helper.get_trees(doc2[train_index:dev_index],
                                            serverState.server_rnn_state.LT,
                                            parserStatistics)
    test_trees = server_rnn_helper.get_trees(
        doc2[dev_index:], serverState.server_rnn_state.LT, parserStatistics)
    print("***trees loaded")
...
def load_model_data_from_file():
    ...
    if is_none([data_file, total_ratio_str, train_ratio_str, dev_ratio_str]):
        return response_error(
            "Load data needs parameters data_file, total_ratio, train_ratio, dev_ratio"
        )
    max_count = -1
...
def train_rnn_impl(trainer, n_epochs):
    trainer.train(
        state=serverState.server_rnn_state,
        rnnWrapper=serverState.rnn,
        n_epochs=n_epochs,
        rng=serverState.rng)
...
def train_rnn():
    ...
    if is_none([
            learning_rate, L1_reg, L2_reg, n_epochs, batch_size,
            retain_probability
    ]):
        return response_error(
            "Training needs parameters learning_rate, L1_reg, L2_reg, n_epochs, batch_size, retain_probability"
        )
    ...
    if not is_positive([trainer.learning_rate, n_epochs, trainer.batch_size]):
        return response_error(
            "Training needs parameters learning_rate, n_epochs, batch_size to be above zero"
        )
...
def response_error(msg):
    res = Response(response=msg, status=500, mimetype="text/plain")
    print(msg)
    sys.stdout.flush()
    return res

...
class Timers:  # for parsing text
    def __init__(self):
        ...
        self.timers = [
            self.totalTimer, self.gettreeTimer, self.nltkTimer, self.normTimer,
            self.ckyTimer
        ]
...
def get_nltk_parsed_tree_from_sentence(l2, parser, timers, parserStatistics):
    ...
    while len(l2) > 0:
        ...
        if tree is not None:
            ...
            print("Warning: No tree obtained from parser. Sentence was: " +
                  tmp)
...
def get_nltk_parsed_trees_from_list(lines):
    ...
    for line in lines:
        ...
        line = line[:
                    -1]  # strips newline. But consider: http://stackoverflow.com/questions/509446/python-reading-lines-w-o-n
        ...
        l2 = l2.replace('\xa0',
                        ' ')  # some leftover non ascii codes xa0=line feed
        tree = None
        try:
...
def getInputArrays(reg, trees, evaluator):
    ...
    x_val = numpy.concatenate(
        list_x, axis=0).reshape(-1,
                                2 * (Evaluator.SIZE + Evaluator.HIDDEN_SIZE))
    ...
    if x_val.shape != (len(list_x), 2 *
                       (Evaluator.SIZE + Evaluator.HIDDEN_SIZE)):
        raise Exception(
            "error in numpy conversion of x, shape was {}".format(x_val.shape))

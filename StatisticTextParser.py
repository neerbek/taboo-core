...
from stat_parser.parser import CKY
...
class StatisticTextParser:
    ...
    def get_tree(self, sent, timers):
        ...
        if i<len(line)-1: #Not all of line parsed
            raise Exception("train parser" + " parsing line failed. There was more than one tree in the line. {}".format(i))
        l2 = load_trees.output(t)    
        if l2!=line: #Lines differ
            raise Exception("train parser" + " marshall and unmarshalling differs" + "\n" + line + "\n" + l2)
        if not t.is_binary():
            raise Exception("train parser" + " tree is not binary")
        if not t.has_only_words_at_leafs():
            raise Exception("train parser" + " tree is not properly normalized")
        return t
